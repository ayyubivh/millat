...
class _LoginViewState extends State<LoginView> {
    ...
    return Scaffold(
      ...
      listener: (context, state) {
        if(state is AuthError){
          ...
          clearDate();
          Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => TabsView(),));
        }
      },
        builder: (context, state) {
            if(state is AuthLoading){
              return Center(child: CircularProgressIndicator(color: green77),);
            }else{
                ...
                child: Column(
                  children: [
                    ...
                    CustomTextField(
                        ...
                        icon: Icon(Icons.email,color: iconsColor), hint: 'Enter your email or Number'),
                    const SizedBox(height: 30,),
                    CustomTextField(
                        controller: _passwordController,
                        icon: Icon(Icons.lock,color: iconsColor), hint: 'Enter your password'),
                    ...
                    MainButton(title: 'Login',onPressed: (){
                      BlocProvider.of<AuthBloc>(context).add(Login(_emailController.text, _passwordController.text));
                    }),
                    const SizedBox(height: 40,),
                    GestureDetector(
                      onTap: (){
  ...
  ScaffoldFeatureController buildError() {
    return ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Please enter username/password')));
  }
